...
### Changed
- **Sidebar slot rows say `slotignored`, never N/A.** A slot has three states: empty, `slotignored` (from the app-type: not required, not scored) and populated. Each row's description now names the state.
...
[0.1.3]: https://github.com/Wolfe-Jam/faf-vscode/releases/tag/v0.1.3
